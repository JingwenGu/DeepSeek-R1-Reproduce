...
dataset = get_gsm8k_questions(split = script_args.dataset_split, num_samples = script_args.test_size)
# endregion
...
def inference(model):
    ...
    device = model.device
    with torch.no_grad():
        for question, answer, prompt in zip(dataset['question'],dataset['answer'],dataset['prompt']):
