...
class ScriptArguments:
    ...
    mix_ratio: Optional[int] = field(default="", metadata={"help": "mix ratio between MATH500 and GSM8K"}) # 20
...
def check_answer(answer,response):
    ...
    try:
        if answer.isdigit():
            ...
            a = latex2sympy(answer).evalf()
            r = latex2sympy(response).evalf()
            return abs(a-r) <= 1e-3
    except:
        return False
...
def correctness_reward_func(prompts, completions, answer, **kwargs) -> list[float]:
    ...
    return [2.0 if check_answer(a,r) else 0.0 for r, a in zip(extracted_responses, answer)]
